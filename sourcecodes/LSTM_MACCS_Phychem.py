...
def train_lstm_model(x_train, x_test, y_train, y_test, model_filename='lstm_maccs_descriptors.keras', plot_filename='lstm_maccs_descriptors.png', epochs=50, batch_size=2):
    ...
    return npv,y_test_pred_prob,accuracy_train, accuracy_test,mcc_train,mcc_test,precision_train,precision_test,recall_train,recall_test, specificity_train,specificity_test,roc_auc_test,roc_auc_train, model_filename, plot_filename
    
def main():
    # Load data from CSV
    x_train = pd.read_csv("irritation_x_train_att_maccs_descriptors.csv",index_col=0)
    x_test = pd.read_csv("irritation_x_test_att_maccs_descriptors.csv",index_col=0)
    y_train = pd.read_csv("irritation_y_train_att_numeric.csv",index_col=0)
    y_test = pd.read_csv("irritation_y_test_att_numeric.csv",index_col=0)
    npv_test,y_test_pred_prob,accuracy_train, accuracy_test,mcc_train,mcc_test,precision_train,precision_test,recall_train,recall_test, specificity_train,specificity_test, roc_auc_test,roc_auc_train, model_file, plot_file = train_lstm_model(x_train, x_test, y_train, y_test)
    # print pred probalibility
    y_test_pred_prob = pd.DataFrame(y_test_pred_prob, columns=['Prediction'], index= y_test.index).to_csv("pred_prob_test_irritation_xgb.csv")
